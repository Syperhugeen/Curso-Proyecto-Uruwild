 <section class="padding-xl" style="background-color:#30302f;">
       <div class="container">
          <div class="row">
             <div class="col-lg-4 col-md-4 text-center padding-xl">
              <img src="{{url()}}/imagenes/Empresa/uruwild_logo_white.png" class="get_width_100">
            </div>
            <div class="col-lg-4 col-md-4 text-center padding-xl border-switch">
              <p class="color-text-gris small"> <i class="fa fa-phone"></i> 2601 1234</p>
              <p class="color-text-gris small"> <i class="fa fa-phone-square"></i> 099 123 456</p>
              <p class="color-text-gris small"><i class="fa fa-envelope"></i> contacto@uruwild.com</p>

               <div class="mx-auto mt-4">
                <span class="col-md-4 mx-auto"><a href=""><span class="socicon socicon-facebook"></span></a></span>
                <span class="col-md-4 mx-auto"><a href=""><span class="socicon socicon-instagram"></span></a></span>
                <span class="col-md-4 mx-auto"><a href=""><span class="socicon socicon-twitter"></span></a></span>
                </div>
            </div> 
            <div class="col-lg-4 col-md-4 text-center padding-xl border-switch">
                <p class="color-text-gris small">URUWILD 2018</p>
                <p class="color-text-gris small">Fotografía: Pablo Donagaray</p>
                <p class="color-text-gris small">Diseño web: <a href="">Simbionte</a> </p>

               
            </div>          
          
          </div>
        </div>
         
</section>